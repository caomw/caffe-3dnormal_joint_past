...
void* ImageLocDataLayerPrefetch(void* layer_pointer)
{
	...
	if (!LocReadImageToDatum(layer->lines_[layer->lines_id_], new_height,
			new_width, &datum))
	{
		...
		vector<Blob<Dtype>*>* top)
{
	...
	const int new_height =
			this->layer_param_.image_loc_data_param().new_height();
	const int new_width = this->layer_param_.image_loc_data_param().new_width();

	const int slide_stride =
			this->layer_param_.image_loc_data_param().slide_stride();
	const int source_num =
			this->layer_param_.image_loc_data_param().source_num();
	...
	Datum datum;
	CHECK(
			LocReadImageToDatum(lines_[lines_id_], new_height, new_width,
					&datum));
	// image
	const int crop_size = this->layer_param_.image_loc_data_param().crop_size();
	const int batch_size =
			this->layer_param_.image_loc_data_param().batch_size();
	const string& mean_file =
			this->layer_param_.image_loc_data_param().mean_file();
	if (crop_size > 0)
	{
		(*top)[0]->Reshape(batch_size, datum.channels(), crop_size, crop_size);
		prefetch_data_.reset(
				new Blob<Dtype>(batch_size, datum.channels(), crop_size,
						crop_size));
	}
	else
	{
		(*top)[0]->Reshape(batch_size, datum.channels(), datum.height(),
				datum.width());
		prefetch_data_.reset(
				new Blob<Dtype>(batch_size, datum.channels(), datum.height(),
						datum.width()));
	}
...
void ImageLocDataLayer<Dtype>::CreatePrefetchThread()
{
	...
	// Create the thread.
	CHECK(
			!pthread_create(&thread_, NULL, ImageLocDataLayerPrefetch<Dtype>,
